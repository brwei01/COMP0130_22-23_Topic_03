...
int main(int argc, char **argv) {
  if (argc != 6) {
    cerr << endl
          << "Usage: " << argv[0] << " settings_files path_to_image_folder path_to_times_file results_file yolov5_detect_result" << endl;
    return EX_USAGE;
  }
  ...
  std::thread runthread([&]() { // Start in new thread
    ...
    for (int ni = 0; ni < nImages; ni++) {
      ...
      string strPathToDetectionResult = argv[5] + std::to_string(vTimestamps[ni]) + ".txt"; // read detect result from yolov5
